...
export const mapAppUriArr = [
  {
    ...
    jumpuri: "amapuri://route/plan/?sid=&slat=${currentLat}&slon=${currentLon}&sname=${currentName}&did=&dlat=${targetLat}&dlon=${targetLon}&dname=${targetName}&dev=0&t=0",
  ...
  },
  {
    ...
    jumpuri: "iosamap://path?sourceApplication=applicationName&sid=&slat=${currentLat}&slon=${currentLon}&sname=${currentName}&did=&dlat=${targetLat}&dlon=${targetLon}&dname=${targetName}&dev=0&t=0",
  ...
  },
  {
    ...
    jumpuri:  "baidumap://map/direction?origin=name:${currentName}|latlng:${currentLat},${currentLon}&destination=${targetName}",
    device:MOBILE_DEVICE.ANDROID
  },
  {
    status:MAP_APP_STATUS.BAIDU_MAP,
  ...
  },
  {
    ...
    jumpuri:"qqmap://map/routeplan?type=drive&from=${currentName}&fromcoord=${currentLat},${currentLon}&to=${targetName}&tocoord=${targetLat},${targetLon}&referer=OB4BZ-D4W3U-B7VVO-4PJWW-6TKDJ-WPB77",
    device:MOBILE_DEVICE.ANDROID
  },
  {
    status:MAP_APP_STATUS.QQ_MAP,
...
export function jumpMap (schemeIntent, jumpuri) {
  ...
  function hasAppPackage () {
    ...
    if (isAndroid) { //安卓手机
      alert("//安卓手机"+sApp)
      sApp = window.startApp.set({ action: "ACTION_VIEW", category: "CATEGORY_DEFAULT", type: "text/css", package: schemeIntent, uri: jumpuri, flags: [ "FLAG_ACTIVITY_CLEAR_TOP", "FLAG_ACTIVITY_CLEAR_TASK" ], intentstart: "startActivity", });
    }
  ...
  //不存在对应APP
  function notAppPackage () { alert("====不存在对应APP") }
}